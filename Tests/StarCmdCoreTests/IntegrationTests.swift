...
struct IntegrationTests {
    ...
    func fullIPCFlow() async throws {
        ...
        {"type":"register","session_id":"integration-test","tmux":"test:main:@1:%10","cwd":"/tmp/test","source":"startup","timestamp":\(Int(Date().timeIntervalSince1970))}
        """)
        try await Task.sleep(for: .milliseconds(100))
        ...
        try await sendMessage(to: socketPath, json: """
        {"type":"notification","session_id":"integration-test","tmux":"test:main:@1:%10","message":"Permission needed","notification_type":"permission_prompt","last_message":"","timestamp":\(Int(Date().timeIntervalSince1970))}
        """)
        try await Task.sleep(for: .milliseconds(100))
        ...
        try await sendMessage(to: socketPath, json: """
        {"type":"notification","session_id":"integration-test","tmux":"test:main:@1:%10","message":"Waiting","notification_type":"idle_prompt","last_message":"What do you want?","timestamp":\(Int(Date().timeIntervalSince1970))}
        """)
        try await Task.sleep(for: .milliseconds(100))
